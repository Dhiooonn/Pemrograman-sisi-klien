...
class Mahasiswa {
    ...
    static deleteById(mahasiswaList, nim) {
        const index = mahasiswaList.findIndex((m) => m.nim === nim);
        if (index !== -1) {
            mahasiswaList.splice(index, 1);
            console.log(`Mahasiswa dengan NIM ${nim} berhasil Dihapus`);
        } else {
            console.log(`Mahasiswa dengan NIM ${nim} tidak ditemuakn`);
        }
    }
...
Mahasiswa.add(mhs1, list.daftarMahasiswa);
Mahasiswa.add(mhs2, list.daftarMahasiswa);
Mahasiswa.add(mhs3, list.daftarMahasiswa);
...
Mahasiswa.update(list.daftarMahasiswa, "A11.2023.11111", { nilaiUAS: 99});
Mahasiswa.deleteById(list.daftarMahasiswa, "A11.2023.222222");

console.log("Setelah Update dan Delete");
list.showAll();
list.clearArray();